...
extern void xprinthex(uint16_t val);
extern void serout(uint8_t value);
extern uint8_t getkey();
extern void clearkey();

//r.b tasmota clash #define NULL (void *)0

bool stop_flag = false; //set to press STOP button
...
//
//	mikeroolz posted this code on the Arduino forum back in 2013. Spasiba!
//	there is no license associated with it from what I can tell
//
...
#define setinterruptX() cpustatus |= FLAG_INTERRUPT
#define clearinterruptX() cpustatus &= (~FLAG_INTERRUPT)
...
void vic20_rtc(void)
{
	...
	jiffies++;
	if (jiffies >= 0x4f1a00) //60 jiffies/sec per day
	{
		jiffies = 0;
	}
	RAM[0xa0] = (jiffies >> 16) & 0xff; //high byte
	RAM[0xa1] = (jiffies >> 8) & 0xff;	//mid byte
	RAM[0xa2] = jiffies & 0xff;			//low byte
}
...
uint8_t read6502(uint16_t address)
{
	...
	//{
	//	BIOSaddr = address - 0xa000;
	//	return basicv5_cart[BIOSaddr];
	//	//return forth_cart[BIOSaddr];
	//}

	if (address >= 0xC000U)
	{
		BIOSaddr = address - 0xc000U;
		return pgm_read_byte(basic + BIOSaddr);
...
void brk()
{
	...
	setinterruptX();			   //set interrupt flag
...
{
	//Serial.printf("Addr: %04X ",pc);
	while (tickcount--)
	{

// traps for the iec bus functions
#define RTS 0x60
		switch (pc)
		{
			//	case 0xef04:
			//		//unlisten
			//		Serial.print("<UNLISTEN>");
			//		opcode = read6502(pc++);
			//		break;
		...
		case 0xee49:
			//send byte
			onSendByte(RAM[0x95], RAM[0x00A3] & 0x80);
			//if (RAM[0x00A3] & 0x80)
			//{
			//	Serial.println("<EOI>");
			//}
		...
		case 0xeed3:
			//wait for bus end
			//Serial.println("<BUSEND>");
			//Serial.println("<ATN Hi>");
			onNotATN();
			opcode = RTS;
			pc++;
			break;
		...
		case 0xef19:
			...
			if (isEOI())
			{
				RAM[0x90] |= 0x40; //flag EOI
								   //Serial.println("<read eoi>");
			}
			//Serial.print("<READ>");
		...
		case 0xf770:
			//keyboard stop key. return Z = 1 if stop is pressed
			if (stop_flag)
			{
				write6502(0x0091, 0xfe); //poke in stop button press
				stop_flag = false;
			}
			else
			{
				write6502(0x0091, 0xff);
			}
